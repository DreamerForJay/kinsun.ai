...
def test_current_approved_dataset_matches_allowlist_and_normalizes_filters() -> None:
    ...
    # 2026-08-02: the not-authorized and pending-revalidation tiers were both
    # promoted, so approved now holds the complete delivered bundle.
    ...
    # The 1966 service guide is a web page, so it has no pagination. A citation
    # must carry both page numbers or neither; a half-filled range points at no
    # location in the source.
    assert sum(1 for document in documents if document["page_start"] is None) == 16
    assert all(
        (document["page_start"] is None) == (document["page_end"] is None) for document in documents
    )
