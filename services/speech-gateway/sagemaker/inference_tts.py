...
Skeleton status: same caveats as inference_asr.py -- written against the
contract and existing adapter code, dry-run locally, never invoked through a
...
def synthesize_bytes(text: str, language: str, speaking_speed: str) -> bytes:
    ...
    locally against sample text (VoxHakka's sixian/hailu dialects; nan is
    expected to raise given the known mms-tts-nan Han-character bug, see
    docs/model-selection.md -- that is documented, not-yet-broken behavior).
