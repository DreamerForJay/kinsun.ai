...
Skeleton status: written against the wire contract and the existing adapter
code, verified with a local dry-run of `transcribe_bytes()` against sample
audio and a `docker build` of the image (see ../docs/sagemaker-endpoint-contract.md
"部署步驟"). Never deployed to or invoked through a real SageMaker endpoint --
this machine has no AWS credentials.
"""
...
def _get_adapter() -> tuple[Any, Any]:
    ...
    if _adapter is None:
        ...
        # Same model + generation-hint mapping voice_server.py uses for its
        # shared en/hak/nan/mixed-zh-nan pipeline: no release of this model
        # has a dedicated nan/hak decoder token, so both fall back to the
        # shared "chinese" hint (docs/model-selection.md has the full story).
        _adapter = TransformersASRAdapter(
            model_id="adi-gov-tw/Taiwan-Tongues-ASR-CE-pretrained-v2.0",
            device=0,
            language_code={"nan": "chinese", "hak": "chinese"},
        )
