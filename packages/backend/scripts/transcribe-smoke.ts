...
const language = languageFlagIndex >= 0 ? process.argv[languageFlagIndex + 1] : undefined;
const jsonOutput = argumentsSet.has('--json');

if (
  !argumentsSet.has('--synthetic') ||
  !pcmPath ||
  (language !== 'zh-TW' && language !== 'en-US')
) {
  throw new Error(
    'Usage: npm run smoke:transcribe -- --audio <synthetic.pcm> --language <zh-TW|en-US> --synthetic [--json]',
  );
}

const audio = await readFile(pcmPath);
if (!jsonOutput) console.log(`準備送出 ${audio.byteLength} bytes 的 Synthetic PCM 音訊`);
...
try {
  ...
  console.log(jsonOutput ? JSON.stringify(output) : output);
} catch (error) {
