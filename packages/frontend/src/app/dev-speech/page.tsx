...
export default function DevSpeechPage() {
  ...
  return (
    <main style={{ padding: 32, maxWidth: 720, fontFamily: 'system-ui, sans-serif' }}>
      ...
      <fieldset style={{ marginBottom: 24, border: '1px solid var(--color-border)', padding: 16 }}>
      ...
      <section style={{ marginBottom: 32 }}>
        ...
        <button
          ...
          style={{
            padding: '12px 24px',
            fontSize: 16,
            background: recording ? 'var(--color-destructive)' : 'var(--color-accent)',
        ...
        {transcript !== '' && (
          <div style={{ marginTop: 16, padding: 16, background: 'var(--color-background)', borderRadius: 6 }}>
            <p style={{ margin: 0, fontSize: 18 }}>{transcript}</p>
            <p style={{ margin: '8px 0 0', color: 'var(--color-muted-foreground)', fontSize: 14 }}>
              confidence {confidence?.toFixed(4)}
              {acceptable === false && (
                <strong style={{ color: 'var(--color-destructive)' }}> — 低於門檻，需請長者再說一次</strong>
              )}
            </p>
          </div>
        )}
